...
class ColorSquares {
  ...
  ColorSquares(DNA _dna)
  {
    ...
    color2 = new color[]{
      #263C8B, #4E74A6, #BDBF78, #BFA524, #2E231F, #D5E1EA, #145281
    };
    color3 = new color[]{
      #e64c3b, #2c3e50, #587c85, #d2ecf1, #f0f0f0, #587c85, #2c3e50
    };
 ...
 colors = color1;
  ...
  void display() {
    ...
    for (int h = 0; h < blockSizes.length; h ++) {
           switch(dna.rnd2[totalRand]){
      case 1: colors = color1;break;
      case 2: colors = color2;break;
      case 3: colors = color3;break;
      case 4: colors = color1;break;
      case 5: colors = color2;break;
      case 6: colors = color3;break;
    }
      // Initialize columns and rows, the number of these will differ depending on the size of the block
      // We will start off with multipling the pixels by an Integer and eventually end up at the single pixel level
